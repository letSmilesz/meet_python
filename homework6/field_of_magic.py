...
def drum(turn):
    ...
    if turn > 3: 
        value = values[randint(0, len(values) - 2)]
        if value == 999: value = choice(prizes)
    else: value = values[randint(0, len(values) - 1)]
    if value is int: print(f'You will have {value} points if you quess the letter!')
    ...
    else: print('You have sector "Plus", you can open any letter!')
    return value
...
for i in range(15):
    ...
    if word.count(letter) > 0:
        ...
        elif value == 'x': points *= word.count(letter)
        else: points = 0
    else: 
